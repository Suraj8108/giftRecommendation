...
export class SearchdashboardComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.idf = history.state.idf
    //console.log(this.category)
    //console.log(this.idf)
    this.searchdashboardservice.idfsearch(this.idf).subscribe((Response)=>{
      this.actual=Response;
      console.log(Response)
    })
  this.searchdashboardservice.getsearch(this.category,this.idf).subscribe((Response)=>{
  ...
  search(){
    ...
    history.state.idf = this.search_resp[0].idf
    //console.log(this.category)
    this.ngOnInit()
    })
  }
